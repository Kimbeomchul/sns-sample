...
public class ChatController {
    ...
    public Flux<ChatEntity> getMsg(@PathVariable String sender, @PathVariable String receiver){ // 해당 api는 1:1 버전 이라 사용하지 않음
    ...
    public Flux<ChatEntity> findByRoomNum(@PathVariable Integer roomNum){ // Flux를 사용하는 이유는 데이터를 끊기지 않고 화면에 보여주기 위해서 넣은거임 흐름이라고 생각하면됨
    ...
    @CrossOrigin
    @PostMapping("/chat")
    public Mono<ChatEntity> setMsg(@RequestBody ChatEntity chatEntity){
